...
```
 ...
 │ 5. BUILDERS       ~30 visões (comerciais + operacionais/qualidade)      │
 ...
 └───────────────────────────────────────────────────────────────────────┘
   ...
   ├─ index.html    (índice central — conecta os 2 painéis)
...
| `operacional.py` | **Builders de qualidade/processo**: completude, origem (migrado×orgânico), status×situação, detectores DQ1/DQ2/DQ3, acionabilidade. |
...
| `report_html.py` | 17 relatórios HTML (Plotly) com insights/recomendações + portais de navegação. |
...
- **Segmentação por público (data products).** Duas trilhas independentes — Comercial e
  Operacional/Qualidade — cada uma com seu workbook, HTMLs, portal, parquet e auditoria.
...
- **Silver** = saneado/conformado (`df_prod_status`, grão de ciclo, flags de último
  ciclo/contato).
- **Gold** = marts por público (as visões comerciais e operacionais).
...
| **Dois públicos separados** | Pedido do gestor; cada audiência recebe só o que lhe importa. |
