...
>
> Geradas em **`outputs/reports/Raio_X_Qualitativo.xlsx`** (6 abas) + Parquets `qual_*` +
> workbooks de auditoria `Qual_*.xlsx` + HTMLs `10`/`11`. O levantamento completo (catálogo
> e tiers de confiabilidade) está em `docs/ANALISES_NAO_MONETARIAS.md`.
...
**Onde:** aba `0_Completude_Cadastro` · parquet `qual_completude_cadastro` · HTML `10_completude_cadastro.html`.
...
**Onde:** aba `1_Mix_Produtos` · parquet `qual_mix_produtos` · auditoria `Qual_Mix_Produtos.xlsx`
(reconciliação: CPFs distintos por produto). **Não usa valor.**
...
**Onde:** aba `2_Market_Share_Contagem` · parquet `qual_market_share_contagem` ·
auditoria `Qual_Market_Share_Contagem.xlsx`.
...
**Onde:** aba `3_Profundidade_Carteira` · parquet `qual_profundidade_carteira` ·
auditoria `Qual_Profundidade_Carteira.xlsx` (reconciliação: produtos distintos por CPF).
...
**Onde:** aba `4_Taxa_Cancelamento` · parquet `qual_taxa_cancelamento`. Sem workbook de
auditoria próprio (é uma razão entre contagens; rastreável via `Calculadora_Produtos.xlsx`).
...
**Onde:** aba `5_CrossSell_Gaps_Especialidade` · parquet `qual_crosssell_gaps` ·
HTML `11_crosssell_gaps.html`. Lastro herdado de `Mix_Especialidade.xlsx`.
...
> Todos os números são **auditáveis** (`outputs/auditoria/`) e **rastreáveis** até a linha de
> origem — sustentam a conversa mesmo com o cadastro imperfeito. Quando os valores forem
> saneados, as visões de prêmio/comissão entram numa 2ª fase (ver `docs/GUIA_METRICAS.md`).
